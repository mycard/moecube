...
export class CubesService {
  ...
  connections = new Map<Cube, Connection>();
  ...
  get lastVisited(): Cube | undefined {
  ...
  set lastVisited(app: Cube | undefined) {
    if (app) {
  ...
  async loadCubes() {
    ...
    try {
      ...
      for (let item of keys_data) {
        let app = data.find((a: any) => a.id === item.app_id);
        if (app) {
          app.key = item.id;
        }
      }
  ...
  async bundle() {
    try {
      // const bundle = require(path.join(remote.currentCube.getPath('appData'), 'moecube', 'bundle.json'));
      // 示例：
      // [
      //     {
      //         "currentCube": "th105",
  ...
  loadAppsList = (data: any): Map<string, Cube> => {
    let apps = new Map<string, Cube>();
    let locale = this.settingsService.getLocale();
    let platform = process.platform;

    for (let item of data) {
      let app = new Cube(item);
  ...
  async importApp(app: Cube, appPath: string, option: InstallOption) {
    if (!app.isInstalled()) {
  ...
  async verifyFiles(app: Cube, checksumFiles: Map<string, string>, callback: () => void): Promise<Map<string, string>> {
  ...
  async update(app: Cube, verify = false) {
  ...
  async doUpdate(app: Cube, changedFiles?: Set<string>, deletedFiles?: Set<string>) {
    if (changedFiles && changedFiles.size > 0) {
      ...
      let updateUrl = Cube.updateUrl(app, process.platform, locale);
  ...
  async install(app: Cube, option: InstallOption) {
      ...
      if (!task.app.readyForInstall()) {
        await new Promise((resolve, reject) => {
          this.eventEmitter.subscribe(() => {
            if (task.app.readyForInstall()) {
              resolve();
            } else if (task.app.findDependencies().find((dependency: Cube) => !dependency.isInstalled())) {
    ...
    const addDownloadTask = async (_app: Cube, dir: string): Promise<{ app: Cube, files: string[] }> => {
      let locale = this.settingsService.getLocale();
      if (!['zh-CN', 'en-US', 'ja-JP'].includes(locale)) {
        locale = 'en-US';
      }
      let metalinkUrl = Cube.downloadUrl(_app, process.platform, locale);
  ...
  async runApp(app: Cube, action_name = 'main') {
    let children = this.findChildren(app);
    let cwd = (<CubeLocal>app.local).path;
    ...
    if (app.id === 'th123') {
      let th105 = <Cube>app.references.get('th105');
      if (th105.isInstalled()) {
        const config_file = path.join((<CubeLocal>app.local).path, 'configex123.ini');
        ...
        config['th105path'] = { path: (<CubeLocal>th105.local).path };
        await new Promise((resolve, reject) => {
  ...
  async network(app: Cube, server: any) {
    if (!this.maotama) {
  ...
  async postInstall(app: Cube, appPath: string) {
  ...
  async getChecksumFile(app: Cube): Promise<Map<string, string>> {
    ...
    let checksumUrl = Cube.checksumUrl(app, process.platform, locale);
