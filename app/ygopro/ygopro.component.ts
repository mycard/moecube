...
export class YGOProComponent implements OnInit, OnDestroy {
  ...
  constructor(private http: Http, public cubesService: CubesService, private loginService: LoginService,
  ...
  async ngOnInit() {
    ...
    this.windbot = (<YGOProData>this.currentCube.data).windbot[locale];

    this.system_conf = path.join(this.currentCube.local!.path, 'system.conf');
    await this.refresh();
  ...
  get_decks(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      fs.readdir(path.join(this.currentCube.local!.path, 'deck'), (error, files) => {
        if (error) {
          resolve([]);
        } else {
  ...
  get_replays(): Promise<string[]> {
    return new Promise((resolve, reject) => {
      fs.readdir(path.join(this.currentCube.local!.path, 'replay'), (error, files) => {
        if (error) {
          resolve([]);
        } else {
  ...
  async delete_deck(deck: string) {
    if (confirm('确认删除?')) {
      await new Promise(resolve => fs.unlink(path.join(this.currentCube.local!.path, 'deck', deck + '.ydk'), resolve));
      return this.refresh();
    }
  }
  ...
  async start_game(args: string[]) {
    ...
    win.minimize();
    return new Promise((resolve, reject) => {
      let child = child_process.spawn(path.join(this.currentCube.local!.path, this.currentCube.actions.get('main')!.execute), args, {
        cwd: this.currentCube.local!.path,
        stdio: 'inherit'
      });
